...
namespace MASTERPM.Model.Commons
{
    public class MASTERHelper
    {
        ...
        public static string SaveBase64ToImage(string base64String, string pathSaveImage)
        {
            ...
            var folderBanner = $"{MASTERConfigs.DirDocumentUpload}/{pathSaveImage}";
            ...
            {
                Image image = MASTERHelper.Base64ToImage(base64String);
                if (image != null)
                {
        ...
        {
            var server = HttpContext.Current.Server;
            var folderFile = $"{MASTERConfigs.DirDocumentUpload}/{pathSaveFile}";
        ...
        public static string CoppyFile(string sourcePath, string targetPath, string fileName, string newFileName = null)
        {
            ...
            {
                throw new Exception(MASTERResources.Instance.Get(MASTERResources.ID.MsgFileUploadNotAccept));
            }
            var server = HttpContext.Current.Server;
            sourcePath = $"{MASTERConfigs.DirDocumentUpload}/{sourcePath}";
            //server.MapPath(String.Concat("~", "/", sourcePath));
            targetPath = $"{MASTERConfigs.DirDocumentUpload}/{targetPath}";
        ...
        public static string CreateFileFromBytes(byte[] fileBytes, string typeData, string AccountID, string fileNameOld, ref string errorMsg)
        {
            if (fileBytes != null && AccountID != null)
            {
                ...
                var folderMedia = $"{MASTERConfigs.DirDocumentUpload}/{pathSaveMedia}";
                ...
                if (typeData.Equals(Constants.WO_TYPE_DETECT_IMAGE))
                {
                    ...
                    {
                        errorMsg = string.Format(MASTERResources.Instance.Get(
                            MASTERResources.ID.MsgErrorImageUrlMaxSize), MASTERHelper.ConvertBytesToMegabytes(
                ...
                else if (typeData.Equals(Constants.WO_TYPE_DETECT_VIDEO))
                {
                    ...
                    {
                        errorMsg = string.Format(MASTERResources.Instance.Get(
                            MASTERResources.ID.MsgErrorVideoUrlMaxSize), MASTERHelper.ConvertBytesToMegabytes(
                ...
                else if (typeData.Equals(Constants.WO_TYPE_DETECT_FILES))
                {
                    ...
                    {
                        errorMsg = string.Format(MASTERResources.Instance.Get(
                            MASTERResources.ID.MsgErrorFileUrlMaxSize), MASTERHelper.ConvertBytesToMegabytes(
        ...
        public static void SendSMS(string phone, string content)
        {
            ...
            {
                Username = MASTERConfigs.AccoutSMS_UserName,
                Password = MASTERConfigs.AccoutSMS_Password
            };
        ...
        {
            if (!string.IsNullOrEmpty(MASTERConfigs.SMS_ForceSendFlg) && MASTERConfigs.SMS_ForceSendFlg.ToString() == "true")
            {
                MASTERHelper.SendSMS(phone, content);
            }
        }
        ...
        public static List<CommonObjVM> ConvertToListLevel(string configLevel)
        {
            ...
            if (!string.IsNullOrEmpty(configLevel))
            {
                ...
                foreach (string item in results)
                {
                    ...
                    x.key = MASTERHelper.ParseInt(ite[0]);
                    x.value = MASTERHelper.ParseInt(ite[1]);
        ...
        {
            var userName = MASTERHelper.GetUserName();
        ...
        public static List<AD_ROLE> GetRoleByChilds(IRepository repository)
        {
            ...
            var listRoleChildTemp = MASTERHelper.TakeListRoleRecursive(roleTemp, roleTemp, roles.ToList());
        ...
        public static List<M_UNIT> GetUnitByChilds(IRepository repository, ref string unitIDLogin)
        {
            ...
            if (MASTERHelper.IsNullGuid(accountID))
            {
                return null;
            }

            var isAdmin = MASTERHelper.CheckRoleAdmin(repository);
            if (isAdmin)
            {
                return units;
            }
            ...
            var listUnitChildTemp = MASTERHelper.TakeListUnitRecursive2(unitTemp, unitTemp, units.ToList());
        ...
        public class MASTERClone<T> where T : class, new()
        {
