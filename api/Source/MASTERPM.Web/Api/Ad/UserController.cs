...
namespace MASTERPM.Web.Api.Ad
{
    ...
    public class UserController : BaseController
    {
        ...
        {
            var passWord = new MASTERCrypto().Encrypt(password);
            ...
            if (userItem == null || string.IsNullOrEmpty(userItem.UserName))
            {
                ...
                {
                    Msg = MASTERResources.ID.MsgLoginFail
                };
            }
            ...
            if (userItem.LockFlg.GetValueOrDefault())
            {
                return new UserInformationVM()
                {
                    Msg = MASTERResources.ID.MsgUserLock
                };
            }
            ...
            if (string.IsNullOrEmpty(userItem.RoleID))
            {
                return new UserInformationVM()
                {
                    Msg = MASTERResources.ID.MsgNotFound
                };
            }
            ...
            return new UserInformationVM()
            {
                ...
                WebPortalUrl = MASTERConfigs.WebPortalUrl,
                WebClientUrl = MASTERConfigs.WebClientUrl,
            };
        }
        ...
        public IHttpActionResult Search(string searchText)
        {
            try
            {
                ...
                var roleByLogins = MASTERHelper.GetRoleByChilds(this.Repository);
                var unitIDLogin = "";
                var unitByLoginsTemp = MASTERHelper.GetUnitByChilds(this.Repository, ref unitIDLogin);
        ...
        public IHttpActionResult Post([FromBody] AD_USER dataRequest)
        {
            try
            {
                ...
                if (isExistUser.Count() > 0)
                {
                    ...
                    var errMsg = string.Format(MASTERResources.Instance.Get(MASTERResources.ID.MsgErrAccountHaveUserExixts));
                ...
                if (dataItem == null)
                {
                    ...
                    {
                        dataItem.Password = (new MASTERCrypto()).Encrypt(dataRequest.Password);
                    }
        ...
        public IHttpActionResult ChangePassword(ChangePasswordVM dataRequest)
        {
            try
            {
                ...
                if (!(new MASTERCrypto()).Encrypt(dataRequest.PasswordOld).Equals(dataItem.Password))
                {
                    return Json(new TResult()
                    {
                        Status = (short)EStatus.Fail,
                        Msg = MASTERResources.Instance.Get(MASTERResources.ID.MsgLoginFail)
                    });
                }

                dataItem.Password = (new MASTERCrypto()).Encrypt(dataRequest.PasswordNew);
        ...
        public IHttpActionResult Logout()
        {
            try
            {
                ...
                this.MASTERLogs.Write(this.Repository, EAction.Insert, JsonConvert.SerializeObject(removeItem));
