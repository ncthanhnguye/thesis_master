...
/// <summary>
/// Summary description for TF_IDF
/// </summary>
public class TFIDFMeasure
{
    //internal static List<int> FindNearestNeighbor(List<int> lstCandidateArticalID, List<KeyPhrase> lstKeyPhrases_Searched, double minDis = 0)
    //{
    //    SortedDictionary<int, List<string>> dic = new SortedDictionary<int, List<string>>();
        
    //}
    //internal static List<KeyPhraseResult> GetConceptContentByKeyPhrases(List<KeyPhrase> lstKeyPhrases_Searched, double minDis = 0)
    //{
    //    // load all concept
    //    List<ConceptInfo> lstAllConcept = ConceptInfo.GetListConcept();
    //    List<KeyPhraseResult> lstResult = new List<KeyPhraseResult>();
    //    KeyPhraseResult rs = new KeyPhraseResult { keys = new List<KeyPhrase>() };

    //    for (int i = 0; i < lstAllConcept.Count; i++)
    //    {
    //        //lstall[i].lstKeyPhrases = ConceptInfo.GetKeyPhrasesByConceptID(lstall[i].ID);
    //        lstResult.Add(new KeyPhraseResult { ID = lstAllConcept[i].ID, keys = ConceptInfo.GetKeyPhrasesByConceptID(lstAllConcept[i].ID) });
    //    }

    //    return FindNearestNeighbors(lstKeyPhrases_Searched, minDis, lstResult);
    ...
    public static List<KeyPhraseResult> FindNearestNeighbors(List<KeyPhraseResult> lstResult, List<KeyPhrase> lstKeyPhrases_Searched, double minDis, int itop)
    {
        ...
        inputs = TFIDFMeasure.Normalize(inputs);
        ...
        {
            lstResult[i].distance = TFIDFMeasure.ComputeCosineSimilarity(search_vector, lstResult[i].vector);
        }
