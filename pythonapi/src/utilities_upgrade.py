...
def Get_Data_Single(url, type):
    ...
    if makeDir == True:
        ...
        if Download_File(pathDownload, url):
            failurePages = []
            inputStream = open(os.path.join(dataFolder, fileName), 'rb')
            inputPdf = PdfFileReader(inputStream, strict = False)
            for i in range(inputPdf.numPages):
                SavePDFIndex(inputPdf, dataFolder, fileName, i)
            q = queue.Queue()
            for i in range(inputPdf.numPages):
...
def OCR_Image(image, typeFile, pageIm):
    ...
    try:
        ...
        image_vertical = cv2.erode(img_bin, ver_kernel, iterations=5) #old iterations=3
        vertical_lines = cv2.dilate(image_vertical, ver_kernel, iterations=5) #old iterations=3
    ...
    if ((typeFile == '0' and cols == 9 and pageIm == '1') or (typeFile == '1' and cols == 8 and pageIm == '1')):
        ...
        try:
            for point in points:
                left, top = point
                # listCheckCells.append(top)
                # if(top != listCheckCells[len(listCheckCells - 1)]):
                #     if(len(listCheckCells) < cols + 1):
                #         failPages.append(pageIm)
                #         resultsOCR["res"] = []
                #         resultsOCR["failPages"] = failPages
                #         return resultsOCR
                ...
                if right and bottom:
                    imgRec = cv2.rectangle(table_image, (left, top), (right, bottom), (0, 0, 255), 3)
...
#     url = os.path.join(DATA_PATH, 'testData', '20221123161205447_daky_nld_160_page_33.jpg')
#     OCR_Image(url, 0, '33')
